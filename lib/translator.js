...
// Modify here
var geomScript = 'openjscad ../CuraEngine/adjust_geometry.jscad'
    ,curaRunScript = '../CuraEngine/build/CuraEngine slice -j ../CuraEngine/resources/machines/fdmprinter.json -e0'
    ,viewerScript = 'python ./yagv/yagv'
    ,inputSTL = ' ./output/test.stl'
    ,outputFile = ' ./output/output.gcode'
//step #0
...
module.exports = function(program){ //program == commandline args
  ...
  geomScript = geomScript + ' -o ' + inputSTL
  ...
  curaRunScript = curaRunScript + ' -l ' + inputSTL + ' -o '+ outputFile
  ...
  // trim new gcode to clean out printed model height
        ...
        gcode1 = fs.readFileSync('./output/test.gcode')
        gcode2 = fs.readFileSync('./output/g2.gcode')

        gcode1 = gcode1 + gcode2

        fs.writeFileSync('output.gcode', gcode1)

        viewerScript = viewerScript + ' output.gcode'//outputFile
        // viewerScript = 'python ./yagv/yagv ../../output/all.gcode'
        var viewer = exec(viewerScript,
          function(error, stdout, stderr) {
